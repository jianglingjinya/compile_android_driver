...
static int ConnectThreadFunction(void *data)
{
	...
	while (KThreadExit)
	{
		...
		if (!ProcessExit)
		{
			...
			{
				if (__builtin_strcmp(task->comm, "PAN") != 0)
					continue;
				...
				ret = get_user_pages_remote(mm, 0x2030000000, num_pages, FOLL_WRITE, pages, NULL);
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(6, 5, 0)	 // 内核 6.5 到 6.12
				ret = get_user_pages_remote(mm, 0x2030000000, num_pages, FOLL_WRITE, pages, NULL);
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0)	 // 内核 6.1 到 6.5
				ret = get_user_pages_remote(mm, 0x2030000000, num_pages, FOLL_WRITE, pages, NULL, NULL);
...
static int do_exit_hook_work(struct pt_regs *regs)
{
	...
	if (__builtin_strstr(task->comm, "pan") != NULL || __builtin_strstr(task->comm, "PAN") != NULL)
	{
		pr_debug("【进程监听】检测到 PAN 进程即将退出！PID: %d, 进程名(comm): %s\n", task->pid, task->comm);
...
static int do_exit_init(void)
{
	...
	pr_debug("成功：inline hook(do_exit) 已安装，开始监听 PAN 退出。\n");
	return 0;
}
